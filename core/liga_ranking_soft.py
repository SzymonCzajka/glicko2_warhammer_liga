...
from glicko2_soft import Player
import os
import glob
...
def save_rankings(filename, players, history, now):
    ...
    out_filename = os.path.join("wyniki_soft", f"ranking_{timestamp}.csv")
...
def get_latest_ranking_file():
    ranking_files = glob.glob("wyniki_soft/ranking_*.csv")
...
def main():
    ...
    # ZAPAMIĘTUJEMY aktualne wartości przed aktualizacją
    rating1_before = p1.getRating()
    rd1_before = p1.getRd()
    rating2_before = p2.getRating()
    rd2_before = p2.getRd()

    # UAKTUALNIAMY obydwu graczy względem danych sprzed gry
    p1.update_player([rating2_before], [rd2_before], [result1])
    p2.update_player([rating1_before], [rd1_before], [result2])
